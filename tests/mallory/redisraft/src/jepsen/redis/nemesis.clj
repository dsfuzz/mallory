...
(defn member-package
  ...
  (when ((:faults opts) :member)
    (let [ops [{:f :join :values [nil]}
               {:f :leave :values [nil]}]]
      (defn dispatch [op test ctx]
        (let [members (set (rdb/members (:db opts) test))
              addable (remove members (:nodes test))
              can-add  (seq addable)
              can-remove (< min-cluster-size (count members))]
          (case (:f op)
            :join   ((fn join [_ _]
                       (if (not can-add)
                         :pending
                         {:type :info, :f :join, :value (or (:value op) (rand-nth (vec addable)))})) test ctx)
            :leave  ((fn leave [_ _]
                       (if (not can-remove)
                         :pending
                         {:type :info, :f :leave, :value (or (:value op) (rand-nth (vec members)))})) test ctx))))
