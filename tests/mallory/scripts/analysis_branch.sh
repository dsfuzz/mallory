...
extract_events_from_logs() {
    ...
    for fuzzer in ${fuzzers[*]}; do
        for ((run = 1; run <= "$iruns"; run++)); do
            ...
            ag "AFLSummary: " "$logfile" | cut -d ":" -f3 | awk 'NR % 3 == 0' >"$tempfile"

            # Calculate the line number
            event_line=0
            while read -r line; do
                # Get the line number
                cum_branches=$(echo "$line" | cut -d "/" -f1 | cut -d " " -f1)
                cum_vectors=$(echo "$line" | cut -d "/" -f2 | cut -d " " -f2)
                {
                    echo "$fuzzer",$event_line,$run,"Branches,$cum_branches"
                    echo "$fuzzer",$event_line,$run,"Vectors,$cum_vectors"
                } >>"$ocsvfile"
...
visualize_events() {
    ...
    python3 /host/tests/mallory/scripts/generate_branch_plot.py --subject "$isubject" --tools "${fuzzers[*]}" --data_csv "$idatafile" --cut_off "$icutoff" --runs "$iruns" --output_csv "$odatafile" --output_fig "$odatafig"
}
