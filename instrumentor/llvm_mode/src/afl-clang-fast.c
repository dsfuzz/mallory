...
static void edit_params(u32 argc, char** argv) {
  ...
  /* We use afl-llvm-pass.so for instrumenting specified events and branch coverage (in default).

     However, instrumeting rust branch coverage, rustc cannot identify global variables. So we use
     trace-pc-guard mode to instrument rust branch coverage.
  */
  if (getenv("USE_TRACE_PC")) {
